...
- **Amendment 3 (2026-09-23, issue #426): the partial-household pattern is measured in the SrV,
  ...
  1. **Measurement (committed 2026-09-23, `scripts/extract_srv_absence.py` at `0f6c49dd`).**
     (Tables first committed in `0f6c49dd`, then regenerated by the #426 final-review fix commit
     with data rows identical to those of `0f6c49dd`, checked by diff; the CSV headers now name
     code state `c7593bc5`, the HEAD at regeneration time, whose working tree carried that
     commit's fixes -- the `0-17` row selected by valid age and a `0-5` + `6-17` = `0-17`
     invariant -- which change no data row.)
  ...
  2. **Corrected premise.** Amendment 2 point 4 and issue #426 as opened state that a child away
     ...
     household with NO other absent adult -- who MAY still be accompanied -- class trips,
     grandparents, the other parent -- the table measures the household pattern, not supervision. 35 (55.17 %) are in a fully
  ...
  4. **Decision: quota, not ban.** The acceptance criterion for a partial-household draw (#426) is:
     ...
     Measured caveat (2026-09-23): (i) the rule is on UNWEIGHTED counts by design, pre-registered
     before the model measurement; (ii) the WEIGHTED SrV share itself lies outside that interval in
     8 of the 27 populated cells of the committed comparison (`absence_composition_vs_srv.csv` of
     the arm-4 manifest), including both the `whole_household` and the `partial_no_absent_adult`
     cell of the `all` row -- the interval is centred off the weighted estimate systematically
     (bands 30-44, 45-64, 65-74 and `all`), not in one cell; the three `0-17` cells are all
     inside; (iii) for `0-17` / `partial_no_absent_adult` the weighted share (25.95 %) sits
     0.34 pp (unrounded) above the lower end (25.60 %; unweighted 26/73 = 35.62 %); (iv) the arm-4
     `whole_household` OUTSIDE verdict depends on this rule (model 61.33 % against an upper end of
     59.22 %), while the `partial_with_absent_adult` OUTSIDE verdict (2.40 % against a lower end of
     9.66 %) does not; (v) a rule on the weighted share with an effective sample size (Kish n_eff,
     which the extraction would then commit as a column) is the alternative the owner may choose -- a
     follow-up issue, not part of this amendment.
  ...
  5. **Model measured on arm 4 (after the fact, 2026-09-23; committed code
     ...
     36.27 % (INSIDE). The small middle share (2.40 %) is the structural gap: the two-stage draw
     produces "part of the household away together" only by chance co-draws. The
     `whole_household` pattern (the household stage plus the few households completed by
     individual co-draws) carries 47.74 % of the 0-5 band's absences
     (`absence_composition_model.csv`, row 0-5), about 1.59 pp of the realised 3.32 % (arm-4
     manifest, pre-registered row 2) -- an upper bound on the household stage's share and the
     traceable form of the superseded "~1.56 %".

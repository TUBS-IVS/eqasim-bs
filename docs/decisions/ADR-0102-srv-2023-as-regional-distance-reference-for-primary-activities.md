...
- **ASSUMPTIONS:**
  ...
  2. GIS-invalid trips (16.9% of 9,730 work candidate trips, 15.7% of 5,449 education candidate
     ...
     03158), and the `inter` scope's aggregate remains a gap, but only by a margin of 0.0003
     (pre-registered EMD 0.085 -> 0.0803) over the 0.08 threshold (zgb EMD 0.0803) -- i.e.
     recovering the tail barely moves either verdict, so the R6 exclusion is not, by itself, the
     dominant driver of the pre-registered gaps (full readout in ADR-0103, section "Amendment 2026-09-04").
...
- **Evidence:** the four committed tables under `eqasim-data/data/braunschweig/srv/`
  (`srv2023_commute_distance_by_kreis.csv`, `srv2023_education_distance_by_kreis_level.csv`,
  `srv2023_commute_distance_quantiles_by_kreis.csv`, `srv2023_commute_distance_sensitivity_by_kreis.csv`), generated by

...
- **Decision:**
  ...
  3. **The target is a MiD × SrV blend with no arbiter**, exactly as ADR-0089's:
     ...
     Wolfsburg (03103) and the `Gesamt` row fall back to MiD (Gesamt never_pt 0.36), the four rural
     disagreement cases take the `mid_shrunk` path. The existing three-group table and target stay
     byte-identical, so the OFF path is unaffected on the data side.
...
- **Consequences:**
  ...
  - **Evidence, and its limits.** The 03101 OFF/ON A/B smoke (2026-08-21, one flag differing;
    ...
    **30.82 %** (OFF, +18.50 pp against the 12.32 % target — reproducing the 100 %-run defect at
    30.17 %, which is what makes the A/B faithful) to **12.17 %** (ON, −0.15 pp). The flatrate
    aggregate, the ONLY simulation-relevant quantity, is preserved: 25.57 % → 25.46 % (0.11 pp).
  ...
  - **ADR-0089 keeps its claim.** Its statement that the split among the non-flatrate types has no
    simulation effect is restated here as TRUE and is the reason this fix is filed as
    reference fidelity; ADR-0089 carries an amendment pointer to this record.

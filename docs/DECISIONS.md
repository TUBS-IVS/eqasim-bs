...
### ADR-0050 · 2026-07-02 · Fleet data-regionalization ceiling + multi-vintage provenance discipline
...
### ADR-0051 · 2026-07-02 · Fleet realism round 2 — whole-model review fixes + three data enhancements

- **Status:** active — implemented on `feature/fleet-quality-and-data` (Plan 3, tasks A1–A4, B1–B6).
- **Numbering caveat:** the `main` / `feature/popsim-optimized-importance` branch carries a DIFFERENT
  ADR-0050 (TAZ per-RS7 gravity friction). This fleet branch's ADR-0050 (data-regionalization ceiling)
  and this ADR-0051 will need renumbering when the fleet branch merges to `main` (resolve the 0050
  collision at merge; keep both — they are unrelated decisions).
- **Context:** After the fleet regionalization (ADR-0050) landed, a data-utilization audit + a
...
- **Evidence:** branch `feature/fleet-quality-and-data` (Plan 3 commits, SDD ledger
  ...
  (`population_explorer.gpkg`, 558,281 households). Follows ADR-0050.

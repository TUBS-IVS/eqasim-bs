...
on 2026-07-22 (PM-layer consolidation). Append-only. Newest on top.
For decisions see docs/DECISIONS.md; for session narrative see SESSION_LOG.md (local).
...
**Update (2026-07-19, #222 -> PR #223 MERGED, `4dc24e5`):** the felix synpp-1.6.2 validation exposed
two synpp execute-config-contract bugs in this stage that would have crashed the server dry-run before
it produced anything -- (1) 2-arg `context.config(key, default)` in the `_active()`/`_inject()`/
`mid_donor.execute()` execute paths (`ExecuteContext.config()` is 1-arg), and (2) `_inject()`->
`external_workplaces._load_gemeinden` reading `germany.population_path`/`_source` which `configure()`
never declared (`PipelineError`). Both masked by the permissive unit-test stubs; caught by the static
contract test + the data-complete felix suite. Fixed by the 1-arg form + declaring the two keys
(mirroring the SvB `incommuters` stage) + a data-independent `configure()`-declares guard. felix full
suite now **3170 passed / 0 failed**. See [[feedback-synpp-execute-config-contract]]. The 1% cordon
dry-run above remains the outstanding validation step.
...
## Appendix: full pre-trim text of the Priority Ranking section (2026-07-22)

PROJECT_BACKLOG.md `## 1. Priority ranking` was condensed to 2-line items on 2026-07-22
(PM-layer consolidation). The full untrimmed text below is kept verbatim so no inline
history (commit hashes, branch names, PR detail, effort/status columns) is lost.

### Full text: agreed next sequence (pre-trim)
...
### Full text: TIER 0-5 tables (pre-trim)

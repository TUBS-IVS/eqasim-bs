...
> Archived verbatim on 2026-08-13 (ADR-0077); superseded by docs/generated/STATUS.md (generated from the registries + resolved config).
> Several rows were already stale when archived (e.g. PR #263 is MERGED,
> not open); do not read any row here as current.
...
- Last full-suite green: **2026-08-13, felix `eqasim` env, 3585 passed / 30 skipped / 0 failed** (on the merged #266-split head, PR #268 / `~/wt-split266`; prior: 2026-08-13 local 3572/32 on the merged #262 head, 2026-08-12 local 3318/32 post-#259-fix).
...
**2 open PRs:** [#263](https://github.com/TUBS-IVS/eqasim-bs/pull/263) — SrV location types #262 (26 commits; 5% A/B validated, suite 3572/0, Fable final review clean); [#271](https://github.com/TUBS-IVS/eqasim-bs/pull/271) — popsim `mid` package split (#267 module 2/7). PR #260 (escort family) + #261 (test-double fixes) MERGED 2026-08-12; PR #268 (chainsolver split #266), #269 (PM closeout) and #270 (#267 module 1/7, enriched stage package — server A/B identical, 3584/1/30 both sides) MERGED 2026-08-13. Local branches not yet merged into `main` (`git branch --no-merged main`), most checked out as `.claude/worktrees/*`:
- `refactor/split-popsim-mid` (worktree `refactor-split-popsim-mid`) — issue [#267](https://github.com/TUBS-IVS/eqasim-bs/issues/267), module 2/7 (`braunschweig/popsim/mid.py`, 1938 lines): split into an 8-submodule helper package (`batch_folders`, `control_cells`, `csv_format`, `donor`, `donor_stratification`, `kreis_controls`, `participation`, `seed_loading`; pure re-export facade, same import path). Two same-namespace name clashes were caught in review and fixed by renaming the NEW module each time: `stratum` → `donor_stratification` (exact-filename clash with the pre-existing `braunschweig/popsim/stratum.py`) and `batches` → `batch_folders` (one character from the facade-re-exported parent alias `mid.batch`). Separately, `seed_loading`'s two large functions were decomposed into 14 named private helpers (e.g. `_read_seed_households`, `_complete_seed_members`; no `_step_*` prefix). Unlike `secondary_chainsolvers`/`enriched`, `mid` is a helper library, not a synpp stage, so it has no `validate()` — cache-neutral by construction (verified: no stage hashes `mid`'s source), and the pre-existing helper-trap gap (`stage.py` never hashes its `mid` helper) is left for module 3. Behavior-preserving (decomposition proven statement-stream-identical under inlining, 46/46 + 20/20), local FULL subset green (342 passed / 1 failed / 2 skipped, known env failure — missing local raw MiD file). **[PR #271](https://github.com/TUBS-IVS/eqasim-bs/pull/271) OPEN**; server A/B gate running.
- `refactor/split-enriched` (worktree `refactor-split-enriched`) — MERGED via PR #270 (#267 module 1/7); dissolve worktree (+ felix review worktrees).

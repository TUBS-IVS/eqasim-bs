...
# --------------------------------------------------------------------------- #
def test_sample_fleet_model_constrained_never_infeasible(default_fleet_sample, ff):
    """In the v2 path, when a drawn (brand, family) is feasibility-known, the
    assigned powertrain must lie in the feasible set.

    Checked on the session's default 32,000-car v2 draw (tests/conftest.py). The
    verdict depends only on (brand, model, powertrain), so each distinct
    combination is checked once: no infeasible combination <=> no infeasible car.
    """
    df_spec, _, _ = default_fleet_sample
    from braunschweig.data.kba.hsn_tsn import canonical_brand, model_family
    combinations = df_spec[["brand", "model", "powertrain"]].drop_duplicates()
    violations = []
    checked = 0
    for brand, model, powertrain in combinations.itertuples(index=False):
        if not model:
            continue
        cb = canonical_brand(brand)
        fam = model_family(cb or "", model)
        feasible = ff.model_feasible_powertrains(brand, fam)
        if feasible is None:
            continue
        checked += 1
        if powertrain not in feasible:
            violations.append((brand, model, powertrain))
    assert checked > 0, "no feasibility-known cars in the sample (test is vacuous)"
    assert not violations, (
        f"{len(violations)}/{checked} feasibility-known (brand, model, powertrain) "
        f"combinations got an infeasible powertrain, e.g. {violations[:5]}")

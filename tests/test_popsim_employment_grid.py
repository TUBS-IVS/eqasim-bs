...
def test_select_load_columns_strips_computed_and_adds_single_year_inputs():
    load_cols = ["HH..", "EMPLOYED_M_young_agg", "EMPLOYED_F_prime_agg"]
    available = ["M_AGE_15", "M_AGE_16", "M_AGE_40", "F_AGE_30", "M_AGE_0_9_agg"]
    result = eg.select_load_columns(
        load_cols, available,
        computed_cols={"EMPLOYED_M_young_agg", "EMPLOYED_M_prime_agg", "EMPLOYED_M_old_agg",
    ...
    assert "EMPLOYED_M_young_agg" not in result
    assert "EMPLOYED_F_prime_agg" not in result
...
def test_select_load_columns_no_duplicate_when_input_already_present():
    load_cols = ["HH..", "M_AGE_40", "EMPLOYED_M_young_agg"]
...
def test_per_cell_targets_3groups_sum_to_kreis_level_times_ageshare():
    # 1 Kreis, 2 cells. Males: cell c1 has 100 prime (age 40), c2 has 300 prime + 50 young (age 20).
    cells = pd.DataFrame({
    ...
    shares = {"03102": {"young": 0.25, "prime": 0.70, "old": 0.05}}
    out = eg.per_cell_employment_targets(cells, census, shares)
    # prime male level = 200*0.70 = 140, split 100:300 -> 35 / 105
    m = out.set_index("ZENSUS100m")
    assert round(out["EMPLOYED_M_prime_agg"].sum(), 6) == 140.0
    assert round(m.loc["c1","EMPLOYED_M_prime_agg"], 6) == 35.0
    assert round(m.loc["c2","EMPLOYED_M_prime_agg"], 6) == 105.0
    # young male level = 200*0.25 = 50, all in c2 -> 50
    assert round(m.loc["c2","EMPLOYED_M_young_agg"], 6) == 50.0
    # female level 0 -> all female columns 0
    assert out["EMPLOYED_F_prime_agg"].sum() == 0.0


def test_per_cell_targets_six_columns_present():
    cells = pd.DataFrame({
    ...
    shares = {"03102": {"young": 0.2, "prime": 0.6, "old": 0.2}}
    out = eg.per_cell_employment_targets(cells, census, shares)
    expected_cols = {f"EMPLOYED_{s}_{g}_agg" for s in "MF" for g in ("young", "prime", "old")}
    assert expected_cols.issubset(set(out.columns))


def test_add_employment_grid_columns_attaches_6_targets_to_cells_copy():
    cells = pd.DataFrame({
        "ZENSUS100m": ["c1", "c2"],
        "KREIS": ["03102", "03102"],
        "M_AGE_40": [100, 300],
    ...
    shares = {"03102": {"young": 0.1, "prime": 0.8, "old": 0.1}}
    out = eg.add_employment_grid_columns(cells, census_levels, shares)
    # All 6 new columns present; original columns preserved.
    for s in "MF":

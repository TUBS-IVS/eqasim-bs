...
# ---------------------------------------------------------------------------
# MiD seed (non-complete-members path): select_seed_columns must retain H_GR
# ---------------------------------------------------------------------------

def test_mid_seed_households_carry_h_gr() -> None:
    """select_seed_columns with extra_household_cols=('H_GR',) must retain H_GR.

    This exercises the NON-complete-members path of the MiD seed: load_mid_seed
    always calls select_seed_columns(..., extra_household_cols=('RegioStaR7', 'H_GR'))
    regardless of the complete_members flag (Task 7 change). We test select_seed_columns
    directly with a minimal synthetic frame so no real 7 GB MiD files are needed.
    """
    from braunschweig.popsim.seed import MID_SEED_COLUMNS, select_seed_columns

    # Minimal MiD households: household ids, weights, and H_GR (declared size).
    ...
    seed_hh, seed_persons = select_seed_columns(
        ...
        persons,
        MID_SEED_COLUMNS,
        extra_household_cols=("H_GR",),
    )

    assert "H_GR" in seed_hh.columns, (
        f"H_GR missing from seed_households columns after select_seed_columns: "
        f"{list(seed_hh.columns)}"
    )
    # Values must be preserved exactly.
    expected = {101: 1, 102: 2, 103: 3}
    actual = seed_hh.set_index("H_ID")["H_GR"].to_dict()
    assert actual == expected, (
        f"H_GR values incorrect: expected {expected}, got {actual}"
    )

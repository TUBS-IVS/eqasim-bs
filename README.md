...
| `departure_time_mapping_min_model_n` | `50` (persons) | The count a ladder rung's **ranking base** — the distribution a person's quantile is computed in — needs before the rung is used; below it at every rung the person stays unmapped and keeps only the de-rounding. The base is the whole population at both call sites: the mapped set itself in the trip build, and the population *ranking context* (every person's raw first departure per cell) in the reporting-day splice, whose own set is far smaller. The realised level split and base are logged per call site. Inert unless the model is `srv_mapped`. |

...
| SrV general day-absence aggregates (committed) | `python scripts/extract_srv_absence.py --raw <srv2023_raw dir> --out-dir eqasim-data/data/braunschweig/srv --source-commit <sha>` (raw SciUse microdata local-only); all four tables come out of one run; the first two are read at run time by the general day-absence model (ADR-0110, below), the last two are the issue-#426 partial-household references and are not read by the pipeline: the composition table is read by `scripts/measure_absence_composition.py`, the partial-subset table by nothing yet | `braunschweig/srv/srv2023_absence_by_age_band.csv`, `braunschweig/srv/srv2023_absence_household_by_size.csv`, `braunschweig/srv/srv2023_absence_composition_by_age_band.csv`, `braunschweig/srv/srv2023_absence_partial_subset_size.csv` |
...
| VRB fare tables (committed, ADR-0133) | Committed with the repository, nothing to download: the 2026 prices come from the VRB Tarifflyer 2026 and the price-stage matrix from the VRB matrix PDF of 2022 (both parsed on the parked branch `codex/regional-pt-fares`; source hashes inside the files and in their data records); the rail single fares are re-derivable with `python scripts/normalize_external_rail_single_fares.py` from the two price-list PDFs under `eqasim-data/external-tariffs/` (optional `pdfplumber`). Read by `braunschweig.matsim.simulation.prepare` because `vrb_zone_fares_enabled` is on | `docs/data/vrb-dated-price-inputs.json`, `docs/data/vrb-price-stage-matrix-2022.csv`, `docs/data/external-regional-rail-single-fares-2026.json` (repository paths, not under `eqasim-data/data`) |
...
| `escort_passive_pair_adult_min_age_years` | `18` (years, > 0) | Minimum age of the household member whose leg a passive escort leg may pair with. Inert while `escort_passive_from_adult` is `false`; ADR-0112 2026-09-17 amendment (issue #409). |
...
`escort_passive_pair_max_gap_minutes` and `escort_passive_pair_adult_min_age_years` need
MiD's `W_ZWECK` vocabulary and (for the pairing) the household, age and departure-time
columns, so the ENTD donor source
...
| `day_absence_escort_protection_enabled` | `true` | Persons with an escort leg in their pre-assignment diary are ineligible for the individual residual stage (issue #425, ADR-0110 Amendment 2): an escort leg evidences presence at home (ADR-0104 Assumption 4), so a person is never both "away all day" and "escorted someone" -- a combination the SrV cannot contain. The household stage stays unprotected (a household that leaves as a whole takes its escorter along). Declares `synthesis.population.trips` as a stage input only when `true`. `false` restores the PR #389 behaviour byte-identically. |

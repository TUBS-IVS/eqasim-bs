...
| SrV general day-absence aggregates (committed) | `python scripts/extract_srv_absence.py --raw <srv2023_raw dir> --out-dir eqasim-data/data/braunschweig/srv --source-commit <sha>` (raw SciUse microdata local-only); all four tables come out of one run; the first two are read at run time by the general day-absence model (ADR-0110, below), the last two are the issue-#426 partial-household reference read by `scripts/measure_absence_composition.py`, not by the pipeline | `braunschweig/srv/srv2023_absence_by_age_band.csv`, `braunschweig/srv/srv2023_absence_household_by_size.csv`, `braunschweig/srv/srv2023_absence_composition_by_age_band.csv`, `braunschweig/srv/srv2023_absence_partial_subset_size.csv` |

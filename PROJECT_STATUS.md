...
**2 open PRs:** [#263](https://github.com/TUBS-IVS/eqasim-bs/pull/263) — SrV location types #262 (26 commits; 5% A/B validated, suite 3572/0, Fable final review clean); [#270](https://github.com/TUBS-IVS/eqasim-bs/pull/270) — enriched stage-package split (#267 module 1/7). PR #260 (escort family) + #261 (test-double fixes) MERGED 2026-08-12, PR #268 (chainsolver split #266) + #269 (PM closeout) MERGED 2026-08-13. Local branches not yet merged into `main` (`git branch --no-merged main`), most checked out as `.claude/worktrees/*`:
- `refactor/split-enriched` (worktree `refactor-split-enriched`) — issue [#267](https://github.com/TUBS-IVS/eqasim-bs/issues/267), module 1/7 (`braunschweig/synthesis/population/enriched.py`, 2890 lines): split into a 6-submodule stage package (`availability`, `base`, `economic_status`, `housing_tenure`, `income_distribution`, `vehicle_ownership`; same synpp module path, full re-export facade, `validate()` cache token) plus `_execute_base` decomposed into 12 named `_step_*` orchestration steps; behavior-preserving, FULL subset green (342 passed / 17 failed / 14 skipped, unchanged failure names). **[PR #270](https://github.com/TUBS-IVS/eqasim-bs/pull/270) OPEN**.

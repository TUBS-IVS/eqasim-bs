...
- `refactor/split-secondary-chainsolvers` (worktree `split-secondary-chainsolvers`) — issue [#266](https://github.com/TUBS-IVS/eqasim-bs/issues/266): 5327-line chainsolver stage split into a 13-submodule stage package (same synpp module path, full re-export facade, `validate()` cache token); behavior-preserving, local subset green (baseline-identical); PR pending user push approval. Follow-up backlog: [#267](https://github.com/TUBS-IVS/eqasim-bs/issues/267) other oversized modules.
- `refactor/split-popsim-mid` (worktree `refactor-split-popsim-mid`) — issue [#267](https://github.com/TUBS-IVS/eqasim-bs/issues/267), module 2/7 (`braunschweig/popsim/mid.py`, 1938 lines): split into an 8-submodule helper package (`batch_folders`, `control_cells`, `csv_format`, `donor`, `donor_stratification`, `kreis_controls`, `participation`, `seed_loading`; pure re-export facade, same import path); `donor_stratification` was named `stratum` until it was renamed to end an exact-filename collision with the pre-existing `braunschweig/popsim/stratum.py`. Separately, `seed_loading`'s two large functions were decomposed into 14 named private helpers (e.g. `_read_seed_households`, `_complete_seed_members`; no `_step_*` prefix). Unlike `secondary_chainsolvers`/`enriched`, `mid` is a helper library, not a synpp stage, so it has no `validate()` — cache-neutral by construction, and the pre-existing helper-trap gap (`stage.py` never hashes `mid`'s source) is left for module 3. Behavior-preserving, FULL subset green (342 passed / 1 failed / 2 skipped, known env failure — missing local raw MiD file). Implemented locally; PR pending.

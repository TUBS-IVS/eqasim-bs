...
| Inner VerBindungen calibration anchor (#193) | `braunschweig.gravity.verbindungen_anchor_enabled` (**default ON**, ADR-0068 override) | `gravity/verbindungen_anchor.py`, `calibration/anchor_holdout.py`, `scripts/run_anchor_holdout.py`, `scripts/diagnose_anchor_p13.py` | 🟢 **BUILT + MEASURED** (branch `feature/verbindungen-anchor`, PR pending): 100pct holdout 2 seeds gate-stable; verdict False → documented human override (5/6 P13 classes + P38 ZGB improve, AO neutral, class-72 shift diagnosed); check B stamped `reference_role=fit` when ON | MiD P13-by-RS7 + P38.2 (independent distance axes); AO margins (corroboration); QZM = fit reference |
...
| Student in-commuters (#140) | `cordon_student_incommuters_enabled` (tri-state, default-ON, gated on `education_gravity_enabled`) | `synthesis/student_incommuters.py`, `data/education/{student_incommuter_counts,student_origins}.py`, `analysis/simwrapper/student_commuters.py` | 🟢 **BUILT** (unit/integration tests green; wired into all 4 scenario writers + 6 all-features run configs); server E2E dry-run (education_gravity ON) still **PENDING**; no real-data validation | LSN SS2025 Hochschule enrollment, DESTATIS 12411-0018 (18-29 pop); origin-Kreis draw is a documented ASSUMPTION (no OD reference), see `docs/features/student-incommuters.md` |
...
- **Worktree `worktree-student-incommuters-140` (2026-07-18, unmerged):** cross-cordon student in-commuter injection (#140 sub-item 2) BUILT: count anchor (enrollment - resident placement), reverse-decay origin-Kreis draw (documented ASSUMPTION, no OD reference), Home->Education->Home frames merged into all 4 scenario writers, OD analysis outputs. Feature-flagged (`cordon_student_incommuters_enabled`, tri-state, default-ON once `education_gravity_enabled` is on); 6 all-features run configs already satisfy the dependency, so the feature resolves ON there by default (documented explicitly in each config now). Local suite green (see `docs/features/student-incommuters.md`). **Remaining:** server E2E dry-run (data-complete env, `education_gravity_enabled=True`) confirming per-commune counts, injected `education` activities, in-ring/gate fallback rate, and `student_commuter_od.csv`; the Home->Education->Home leg timing uses raw HTS donor times without gate-teleport re-seeding (v1 simplification, carried over from the SvB in-commuter stage) — validate implied speed before any scientific use of trip-duration output.

...
def impute_chain_times(
    ...
    *,
    ...
    if valid.empty:
        ...
        logger.warning(
            "[popsim.time_imputation] no valid persons available to build the "
            "empirical pools; all %d candidates are skipped (routed to stage B).",
    ...
    for person_id in sorted(groups):
        ...
        for _ in range(max_attempts):
            departure = float(rng.choice(first_departure_pool))
            departures, arrivals = [], []
            for i, duration_s in enumerate(trip_durations_s):
                ...
                if i + 1 < len(trip_durations_s):
                    ...
                    if len(pool) == 0:
                        ...
                        departures = None
                        break
                    departure = arrival + float(rng.choice(pool))
    ...
    logger.info(
        ...
        "%d attempts, %d absent from the table).",

...
from braunschweig.popsim.kreis_attribute_control import (
    EDUCATION_AGE_BOUNDS, EDUCATION_FLAG_CATEGORIES, EDUCATION_FLAG_EDU,
    EDUCATION_FLAG_NOEDU, WORK_BY_EMPLOYMENT_CATEGORIES,
...
def derive_education_flag_seed(persons, wege, *, escort_passive_education,
    ...
    flag = _plan_source_flag(persons, wege, codes, exclude_rbw_legs=exclude_rbw_legs,
                             household_id=household_id, person_id=person_id,
                             name="education_flag")
    # Read the two labels BY NAME, not by tuple position: a display reorder of
    # EDUCATION_FLAG_CATEGORIES must not be able to invert the seed.
    edu_label, noedu_label = EDUCATION_FLAG_EDU, EDUCATION_FLAG_NOEDU
    out = persons.copy()

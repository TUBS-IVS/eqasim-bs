...
# Minimum size of a ladder rung's RANKING BASE -- the distribution a person's quantile is computed
# in -- for that rung to be used; a thinner one climbs to the next rung instead of being ranked
# against the reference on its own. ONE rule at both call sites (rulings A-R18 / A-R20): the base is
# the model population of that rung's own key, arriving as the mapped set itself in the
# whole-population trip build and as the POPULATION ranking context in the reporting-day splice.
# Clamped to >= 1 in the model, as min_reference_n is: a rung with an EMPTY base is never usable.

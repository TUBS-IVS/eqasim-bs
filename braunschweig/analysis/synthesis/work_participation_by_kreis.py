...
#: ``R`` / ``_srv_distance_targets`` / ``_srv_work_participation`` own the committed SrV
#: reference, the Kreis set and the distance-class bands every row of this comparison is
#: measured against; all three were module-level imports outside the token (#327 helper-hash
#: re-audit). ``_day_absence`` is folded in because :func:`execute` directly calls
...
#: ``braunschweig.analysis.spatial`` and ``braunschweig.provenance`` are imported INSIDE

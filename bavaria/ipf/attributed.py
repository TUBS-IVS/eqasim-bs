...
(``bavaria.ipf.use_household_size_margin``), this stage also performs a
...
def _form_households(df: pd.DataFrame, random_seed: int) -> pd.DataFrame:
    ...
    print(
        f"[bavaria.ipf.attributed] formed {n_hh:,} households from "
...
def _assign_household_types(
    ...
    if hh_df["hh_size_bin"].isna().any():
        ...
        raise RuntimeError(
            f"[bavaria.ipf.attributed] unmapped household_size in hh_type "
    ...
    if df["hh_type"].isna().any():
        ...
        raise RuntimeError(
            f"[bavaria.ipf.attributed] {n_missing:,} persons missing hh_type "
    ...
    print(
        f"[bavaria.ipf.attributed] assigned hh_type to {n_hh:,} households; "
...
def configure(context):
    ...
    context.config("bavaria.ipf.use_household_type_margin", False)
    if context.config("bavaria.ipf.use_household_type_margin"):
...
def execute(context):
    ...
    use_hh_type = context.config("bavaria.ipf.use_household_type_margin")
    if use_hh_type and not use_hh_size:
        raise RuntimeError(
            "[bavaria.ipf.attributed] use_household_type_margin requires "

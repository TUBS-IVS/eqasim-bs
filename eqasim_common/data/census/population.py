...
def configure(context):
    ...
    context.config("braunschweig.population_path", "bavaria/a1310c_202200.xla")
...
def execute(context):
    df_census = pd.read_excel("{}/{}".format(
        context.config("data_path"), context.config("braunschweig.population_path")
...
def validate(context):
    if not os.path.exists("{}/{}".format(context.config("data_path"), context.config("braunschweig.population_path"))):
        raise RuntimeError("Bavarian census data is not available")

    return os.path.getsize("{}/{}".format(context.config("data_path"), context.config("braunschweig.population_path")))

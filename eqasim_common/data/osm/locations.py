...
def configure(context):
    context.stage("eqasim_common.data.osm.chunked")
    context.stage("data.spatial.municipalities")
    context.stage("data.spatial.iris")

def execute(context):
    df_zones = context.stage("data.spatial.iris")[["geometry", "commune_id", "iris_id"]]
    chunk_ids = context.stage("eqasim_common.data.osm.chunked")
    ...
    for osm_filter in OSM_FILTERS:
        ...
        for chunk in context.progress(chunk_ids, total = len(chunk_ids)):
            osm = pyrosm.OSM("{}/{}.osm.pbf".format(context.path("eqasim_common.data.osm.chunked"), chunk))

"""Extract the MiD 2023 'monatliches HH-Nettoeinkommen x Haushaltsgroesse x Region' tables.

Source (local-only raw xlsx, exported from the MiD 2023 online table tool,
sheet ``MiD Tabellen``):

  * ``mid2023_income_by_size_bundesland.xlsx``
        16 stacked blocks, one per Bundesland (header row
        ``Bundesland: - <name>``). Within a block: columns =
        Haushaltsgroesse in Gruppen (1 / 2 / 3 / 4 / 5+ Personen), rows =
        ``monatliches HH-Nettoeinkommen in 10 Gruppen`` (10 brackets). Each size
        column gives P(income_bracket | hh_size, Bundesland) (Spalten %, column %
        summing to 100 over brackets); the ``Basis gewichtet`` row gives the
        (hh_size, Bundesland) weighted household base.
  * ``mid2023_income_by_size_raumtyp.xlsx``
...
Both tables encode P(income_bracket | hh_size, region). The Bundesland table is
the BASE (Niedersachsen) and the raumtyp table is a within-NDS tilt downstream
(see braunschweig.data.mid.income_by_size).

Output (committed tidy CSVs, long form):

    eqasim-data/data/braunschweig/mid/mid2023_income_by_size_bundesland.csv
        columns [region, hh_size, income_bracket, share_pct, base_weighted]
    eqasim-data/data/braunschweig/mid/mid2023_income_by_size_raumtyp.csv
        columns [region, hh_size, income_bracket, share_pct, base_weighted]

The per-bracket EUR bound table lives in code
(``braunschweig.data.mid.income_by_size.INCOME_BRACKET_BOUNDS_EUR``), with the
open top bracket "mehr als 7000" -> (7000, None); see that module for the
open-top handling. It is also re-emitted as a committed companion CSV
(``mid2023_income_bracket_bounds_eur.csv``) for provenance / downstream readers.
...
Regenerate with:
    ...
    python scripts/extract_mid_income_by_size.py --git-add

Provenance: MiD 2023 (BMDV / infas), regional sample online table tool, table
"monatliches HH-Nettoeinkommen in 10 Gruppen" cross-tabulated by
Haushaltsgroesse x Bundesland and by Haushaltsgroesse x Raumtyp, exported
2026-06.
"""
...
_INCOME_ROW_MARKER = "monatliches hh-nettoeinkommen"
_TOTAL_MARKER = "total"
...
def _region_for_header(cell: str, marker: str,
    ...
    The header is e.g. 'Bundesland: - Niedersachsen' or
    'zusammengefasster regionalstatistischer Raumtyp (7 Kategorien): -
    Stadtregion - Metropole'. The region name follows the FIRST ':' and a
    leading '-'; the rest is the (possibly hyphenated) region label.
    """
    low = _normalise_ws(cell).lower()
    if marker not in low:
        return None
    raw = _normalise_ws(cell)
    # Split on the first ':' so hyphenated region names (e.g. 'Stadtregion -
    # Metropole') stay intact; then strip the single leading '-'.
...
def _locate_size_columns(df_raw: pd.DataFrame, start: int, end: int) -> dict[int, str]:
    """Locate the column index -> hh_size key map for one block.

    The size labels sit on the row AFTER the 'Spalten % (gewichtet)' header row.
    """
    for i in range(start, end):
        if _normalise_ws(df_raw.iloc[i, 0]).lower() == _COLUMN_HEADER_MARKER:
            ...
            for c in range(2, df_raw.shape[1]):
                label = _normalise_ws(hdr.iloc[c])
                if label in SIZE_LABEL_TO_KEY:
                    found[c] = SIZE_LABEL_TO_KEY[label]
            if found:
                return found
    raise RuntimeError(
        f"extract_mid_income_by_size: no Haushaltsgroesse column header in block "
        f"rows {start}..{end}."
    )
...
def _read_base_weighted(df_raw: pd.DataFrame, start: int, end: int,
    ...
    raise RuntimeError(
        f"extract_mid_income_by_size: 'Basis gewichtet' row missing in block "
        f"rows {start}..{end}."
    )
...
def _block_bounds(df_raw: pd.DataFrame, marker: str,
    ...
    if unknown:
        raise RuntimeError(
            "extract_mid_income_by_size: unmapped region header(s) "
            f"{unknown}. Update the region label map."
        )
    if not starts:
        raise RuntimeError(
            f"extract_mid_income_by_size: no region block headers found "
            f"(marker '{marker}')."
        )
    bounds = []
...
def parse_sheet(df_raw: pd.DataFrame, marker: str,
    ...
    """Parse one income-by-size sheet into tidy long form.

    Returns ``(tidy_df, coercion_counts)``. ``share_pct`` is the verbatim MiD
    column-% P(income_bracket | hh_size, region); ``base_weighted`` is the
    (hh_size, region) weighted household base (repeated across bracket rows).
    """
    coercion = {"value": 0, "suppression": 0, "blank": 0}
    records = []

    for start, end, region in _block_bounds(df_raw, marker, region_label_to_key):
        cols = _locate_size_columns(df_raw, start, end)
    ...
    tidy = (
        ...
        .sort_values(["region", "hh_size", "income_bracket"])
        .reset_index(drop=True)
    )
    return tidy, coercion


def _bracket_order_index(bracket: str) -> int:
    return INCOME_BRACKET_CATEGORIES.index(bracket)
...
def main() -> None:
    ...
    bund_xlsx = mid_dir / "mid2023_income_by_size_bundesland.xlsx"
    raum_xlsx = mid_dir / "mid2023_income_by_size_raumtyp.xlsx"
    ...
    print(
        f"[extract_mid_income_by_size] {bund_xlsx.name}: {len(tidy_bund)} rows, "
        f"{tidy_bund['region'].nunique()} regions x {tidy_bund['hh_size'].nunique()} "
        f"sizes x {tidy_bund['income_bracket'].nunique()} brackets; "
    ...
    print(
        f"[extract_mid_income_by_size] {raum_xlsx.name}: {len(tidy_raum)} rows, "
        f"{tidy_raum['region'].nunique()} regions x {tidy_raum['hh_size'].nunique()} "
        f"sizes x {tidy_raum['income_bracket'].nunique()} brackets; "
        f"coercion: numeric={coerce_raum['value']}, "
        f"suppression-token={coerce_raum['suppression']}, blank={coerce_raum['blank']}."
    )

    out_bund = mid_dir / "mid2023_income_by_size_bundesland.csv"
    out_raum = mid_dir / "mid2023_income_by_size_raumtyp.csv"
    out_bounds = mid_dir / "mid2023_income_bracket_bounds_eur.csv"

    header_common = (
        "# Generated by scripts/extract_mid_income_by_size.py from\n"
        "# {src} (sheet 'MiD Tabellen').\n"
        "# share_pct = P(income_bracket | hh_size, region) in percent (a size\n"
        "# column sums to ~100 over brackets); base_weighted = weighted household\n"
        "# base of the (hh_size, region) cell. Bracket EUR bounds: see\n"
    ...
    for out_csv, src, tidy in (
        ...
        with open(out_csv, "w", encoding="utf-8", newline="") as handle:
            handle.write("# MiD 2023 monatliches HH-Nettoeinkommen x Haushaltsgroesse x Region.\n")
            handle.write(header_common.format(src=src))
            tidy.to_csv(handle, index=False)
        print(f"[extract_mid_income_by_size] wrote {out_csv}")

    # Companion EUR bound table (provenance / downstream readers). Open top
    # bracket -> empty high_eur cell (parsed as NaN -> None).
    bounds_records = [
        {
            "income_bracket": bracket,
            "order": _bracket_order_index(bracket),
            "low_eur": low,
            "high_eur": "" if high is None else high,
        }
        for bracket, (low, high) in INCOME_BRACKET_BOUNDS_EUR.items()
    ]
    bounds_df = (
        pd.DataFrame.from_records(
            bounds_records, columns=["income_bracket", "order", "low_eur", "high_eur"]
        )
        .sort_values("order")
        .reset_index(drop=True)
    )
    with open(out_bounds, "w", encoding="utf-8", newline="") as handle:
        handle.write(
            "# MiD 2023 monthly net household income bracket EUR bounds.\n"
            "# Source of truth: braunschweig.data.mid.income_by_size."
            "INCOME_BRACKET_BOUNDS_EUR.\n"
            "# Half-open [low_eur, high_eur). The top bracket 'over_7000' is\n"
            "# open-ended (empty high_eur); a finite EUR value is drawn for it\n"
            "# with a heavy-tail rule (see the enrichment stage).\n"
        )
        bounds_df.to_csv(handle, index=False)
    print(f"[extract_mid_income_by_size] wrote {out_bounds}")

    if args.git_add:
        import subprocess
        for out_csv in (out_bund, out_raum, out_bounds):
            subprocess.run(["git", "add", "-f", str(out_csv)], cwd=str(REPO), check=True)
        print("[extract_mid_income_by_size] git add -f 3 CSVs")

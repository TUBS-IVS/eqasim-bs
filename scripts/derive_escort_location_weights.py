...
def derive_distance_factors(df: pd.DataFrame,
    ...
    construction, see ``test_category_map_covers_all_bhol_codes``). Categories
    with n == 0 in the GIS-valid subset (``sub``) land in ``absent_categories``,
    regardless of whether they hold any valid-BHOL legs at all: a category
    with NO valid-BHOL legs either (denominator 0 too) reports NaN, while a
    category with valid-BHOL legs but zero GIS-valid ones carries
    ``gis_coverage_weighted = 0.0`` (not NaN) -- a distinct, informative
    signal from a truly absent category rather than a silent drop.
    """
...
def main(argv=None) -> int:
    ...
    if not args.mid_wege_path.exists():
        raise FileNotFoundError(
            f"[derive_escort_location_weights] MiD Wege input not found: "
            f"{args.mid_wege_path} (required for the SrV-vs-MiD length "
            "coherence gate; see compute_length_coherence)."
        )
    ...
    args.distance_factors_output.parent.mkdir(parents=True, exist_ok=True)
    ...
    logger.info(
        f"[derive_escort_location_weights] distance factors -> "

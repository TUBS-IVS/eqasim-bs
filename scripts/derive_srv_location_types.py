...
def main(argv=None) -> int:
    ...
    mode_rate = stats["n_excluded_invalid_mode"] / stats["n_in_scope"]
    length_rate = stats["n_excluded_invalid_length"] / stats["n_in_scope"]
    logger.info(
        "[derive_srv_location_types] in-scope legs %d, excluded invalid mode "
        "%d/%d (%.2f%%), excluded invalid length %d/%d (%.2f%%), thin cells "
        "omitted %d, cells reported %d (min_obs=%d)",
        stats["n_in_scope"],
        stats["n_excluded_invalid_mode"], stats["n_in_scope"], 100.0 * mode_rate,
        stats["n_excluded_invalid_length"], stats["n_in_scope"], 100.0 * length_rate,
        stats["n_thin_cells"], stats["n_cells_reported"], stats["min_obs"],
    )
    ...
    probs_header = (
        ...
        f"n_in_scope={stats['n_in_scope']}; exclusion rates are counted against "
        f"n_in_scope: excluded_invalid_mode={stats['n_excluded_invalid_mode']}/"
        f"{stats['n_in_scope']} ({100.0 * mode_rate:.2f}%), "
        f"excluded_invalid_length={stats['n_excluded_invalid_length']}/"
        f"{stats['n_in_scope']} ({100.0 * length_rate:.2f}%); "
    ...
    shares_header = (
        ...
        "# COLUMN-UNIVERSE ASYMMETRY (read this before using the table): "
        "weight_share and n_legs_unweighted are computed over EVERY in-scope "
        "leg for the category (its V_ZWECK code alone decides inclusion); "
        "weighted_median_gis_km and weighted_median_euclid_km are computed "
        "only over the subset of that same category with a valid GIS length "
        "(GIS_LAENGE_GUELTIG > 0) -- narrowing the share itself to that subset "
        "would silently bias the category mix towards better-reported "
        "categories, so the two column groups intentionally use different "
        "denominators; per-category length-validity coverage is logged by the "
        "generating script (see console log), not persisted in this file.\n"

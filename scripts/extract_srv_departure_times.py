...
def _departure_time_diagnostics(table: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "n_legs_unknown_purpose_raw": attrs["n_legs_unknown_purpose_raw"],
        "n_legs_unknown_purpose_valid_dep_min": attrs["n_legs_unknown_purpose_valid_dep_min"],
    }
...
def _activity_duration_diagnostics(table: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "n_legs_unknown_purpose_raw": attrs["n_legs_unknown_purpose_raw"],
        "n_legs_unknown_purpose_included": attrs["n_legs_unknown_purpose_included"],
    }
...
def _departure_time_header(table: pd.DataFrame, diagnostics: dict, headline: dict,
    ...
    lines = [
        ...
        % source_commit,
        "#   (the commit that INTRODUCED this module; the table shape/header were updated in a",
        "#   later fix-round commit -- the measured shares are unchanged where the cell existed).",
        "# Table: %s" % D.DEPARTURE_TIME_TABLE,
        "# Role: the SrV side of the departure-time model (issue #123, ADR-0114). Position",
        "#   'first' is the model's CALIBRATED target (spec",
        "#   2026-09-09-departure-time-srv-mapping-design.md section 2.2): 'srv_mapped'",
        "#   quantile-maps a person's de-rounded FIRST departure onto this distribution's",
        "#   (purpose, group) cell. Positions 'later' / 'all' are the model's HOLD-OUT (validation)",
        "#   reference -- later trips are never mapped. NOT a control target for PopulationSim.",
        "#   Read by the departure-time model AND by the planned analysis stage",
        "#   braunschweig.analysis.synthesis.departure_time_vs_srv (spec section 2.1.3).",
        ...
        "#   is keyed on (purpose, group) only). A person whose group is outside this list makes",
        "#   the extraction RAISE naming the value (srv_departure_times._attach_group).",
        "# Purposes: %s (%s is a POOLED purpose: every leg of the" % (list(D.PURPOSES_WITH_ALL), D.PURPOSE_ALL),
        "#   segment/position regardless of its own purpose, so the departure-time model's",
        "#   coarsening ladder for a thin cell -- (purpose, group) -> (purpose, 'all' segment) ->",
        "#   ('all' purpose, 'all' segment) -- is fully reachable on this file). A leg whose",
        "#   destination purpose harmonise_srv could not map to a named purpose (harmonise_srv",
        "#   does NOT exclude such legs -- it only cannot classify them) is EXCLUDED from every",
        "#   named-purpose cell and INCLUDED only in the '%s' pooled cells -- see the" % D.PURPOSE_ALL,
        "#   'unknown-purpose' exclusion line below.",
        ...
        "#   DENSE over bin_15min -- every (segment, purpose, position) cell carries all %d bins,"
        % D.N_BINS,
        "#   0.0 included, never omitted. A cell with ZERO contributing legs is still EMITTED, with",
        "#   n_unweighted 0 and BOTH share columns NaN for every one of its bins (never a",
        "#   fabricated 0.0, never a dropped row -- the same convention as",
        "#   srv_absence.build_absence_household_by_size's per-size-class rows). bin_15min =",
        "#   floor(minute_of_day / %d), 0..%d (0-28 h). share_derounded and, independently,"
        % (D.BIN_MINUTES, D.N_BINS - 1),
        "#   share_as_reported each sum to 1.0 over a NON-EMPTY (segment, purpose, position)",
        ...
        "#   the leg contributes to (its own segment AND 'all'; its own purpose-cell AND the",
        "#   pooled '%s'; its own position AND 'all')." % D.PURPOSE_ALL,
        ...
        "# Departure-time exclusions (this table, on the harmonised universe, unknown-purpose",
        "#   counts are informational -- those legs are NOT dropped, only excluded from named-",
        "#   purpose cells): "
        + ", ".join("%s=%s" % (k, v) for k, v in diagnostics.items()),
        "# Rows: %d (%d segments x %d purposes incl. '%s' x %d positions x %d bins, DENSE)."
        % (len(table), len(D.SEGMENTS), len(D.PURPOSES_WITH_ALL), D.PURPOSE_ALL, len(D.POSITIONS),
           D.N_BINS),
    ]
    return lines
...
def _activity_duration_header(table: pd.DataFrame, diagnostics: dict,
    ...
    lines = [
        ...
        "# Role: the HOLD-OUT (validation) reference for the departure-time model (issue #123,",
        "#   ADR-0114) in its ENTIRETY -- activity durations are never mapped or touched by the",
        "#   model (spec 2026-09-09-departure-time-srv-mapping-design.md section 2.2); this table",
        "#   only measures whether the model's realised durations still resemble SrV's after the",
        "#   first-departure mapping changes upstream chain timing. Read by the planned analysis",
        "#   stage braunschweig.analysis.synthesis.departure_time_vs_srv (spec section 2.1.3), not",
        "#   by the model itself.",
        ...
        "# Segments: %s." % (list(D.SEGMENTS),),
        "# Purposes: %s (%s is a POOLED purpose: every leg of the segment, regardless of its own"
        % (list(D.PURPOSES_WITH_ALL), D.PURPOSE_ALL),
        "#   purpose -- same rationale as the departure-time reference's pooled purpose). A leg",
        "#   whose destination purpose harmonise_srv could not map to a named purpose (it does NOT",
        "#   exclude such legs, it only cannot classify them) is EXCLUDED from every named-purpose",
        "#   cell and INCLUDED only in the '%s' pooled cell -- see the 'unknown-purpose'" % D.PURPOSE_ALL,
        "#   exclusion line below. Otherwise, purpose is the CURRENT trip's destination purpose,",
        "#   i.e. the activity that trip leads into. Long format: one row per (segment, purpose,",
        "#   band), DENSE over band for any (segment, purpose) with >= 1 included leg -- all %d of"
        % len(D.DURATION_BAND_LABELS),
        "#   %s, most 0.0; share sums to 1.0 per (segment, purpose); n_unweighted repeats the"
        % (list(D.DURATION_BAND_LABELS),),
        "#   group's unweighted leg count. UNLIKE the departure-time reference, a (segment,",
        "#   purpose) with ZERO contributing legs is OMITTED entirely (this table is read by",
        "#   humans and the analysis stage, not programmatically coarsened by the model).",
        ...
        "# Harmonisation exclusions (srv_plan_structure.harmonise_srv, full raw microdata): "
        + ", ".join("%s=%s" % (k, v) for k, v in harmonisation_diagnostics.items()),
        "# Activity-duration exclusions (this table, on the harmonised universe, unknown-purpose",
        "#   counts are informational -- those legs are NOT dropped, only excluded from named-",
        "#   purpose cells): "
        + ", ".join("%s=%s" % (k, v) for k, v in diagnostics.items()),
        "# Rows: %d." % len(table),
...
def _check_departure_time_invariants(table: pd.DataFrame) -> None:
    ...
    bad_purpose = set(table["purpose"]) - set(D.PURPOSES_WITH_ALL)
    if bad_purpose:
        raise ValueError("unexpected purpose value(s) %s" % bad_purpose)
    if D.PURPOSE_ALL not in set(table["purpose"]):
        raise ValueError("the pooled purpose '%s' is missing from the table entirely" % D.PURPOSE_ALL)
    if not table["bin_15min"].between(0, D.N_BINS - 1).all():
        raise ValueError("bin_15min outside [0, %d] present" % (D.N_BINS - 1))

    # DENSE: every (segment, purpose, position) triple must carry exactly N_BINS rows, and the
    # table as a whole exactly segments x purposes x positions x N_BINS rows -- a hole here would
    # silently break Task 3's load_departure_time_reference (review ruling A-R9).
    expected_rows = len(D.SEGMENTS) * len(D.PURPOSES_WITH_ALL) * len(D.POSITIONS) * D.N_BINS
    if len(table) != expected_rows:
        raise ValueError(
            "expected a dense table of %d rows (%d segments x %d purposes incl. '%s' x %d "
            "positions x %d bins), found %d"
            % (expected_rows, len(D.SEGMENTS), len(D.PURPOSES_WITH_ALL), D.PURPOSE_ALL,
               len(D.POSITIONS), D.N_BINS, len(table)))
    sizes = table.groupby(["segment", "purpose", "position"]).size()
    bad_sizes = sizes[sizes != D.N_BINS]
    if len(bad_sizes):
        raise ValueError("expected exactly %d bins per (segment, purpose, position); found a "
                         "different count for %d triple(s): %s"
                         % (D.N_BINS, len(bad_sizes), bad_sizes.head().to_dict()))

    # An empty cell (n_unweighted == 0) must carry NaN in both share columns, never a fabricated
    # 0.0 -- and a non-empty cell must not carry a stray NaN either.
    empty = table[table["n_unweighted"] == 0]
    if len(empty) and not (empty["share_derounded"].isna().all()
                          and empty["share_as_reported"].isna().all()):
        raise ValueError("a cell with n_unweighted == 0 has a non-NaN share; empty cells must "
                         "carry NaN shares, never a fabricated 0.0")
    ...
    for column in ("share_derounded", "share_as_reported"):
        ...
        if len(off):
            raise ValueError("%s does not sum to 1.0 for %d non-empty (segment, purpose, "
                             "position) triple(s): %s"
                             % (column, len(off), off.head().to_dict()))
